-- Migration 034: Update consume_credit_on_submission to store bundle_id
-- Sets submissions.bundle_id to track which bundle paid for each submission
  ...
  IF (TG_OP = 'UPDATE' AND OLD.status = 'draft' AND NEW.status = 'submitted') OR
    ...
    ELSE
      ...
      IF v_bundle_id IS NOT NULL THEN
        -- Store which bundle paid for this submission (for guarantee eligibility tracking)
        NEW.bundle_id := v_bundle_id;
