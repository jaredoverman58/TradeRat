...
### Recent completions (session 2026-08-21)
- **Phase 0 - Pricing fixes:** Updated Counter Offer prices to $5.49 (Standard) and $6.49 (Rat Rate). Reduced multi-pack prices: Standard 3-pack $9.99, Rat Rate 3-pack $12.99, Standard 5-pack $16.99, Rat Rate 5-pack $21.99. Removed all "Save $X" language from pricing page. Removed mixed bundles from pricing page. Updated lib/bundles.ts, app/pricing/page.tsx, landing page pricing table via migration 025, and admin analytics/payments tracking. Created migration 025_update_counter_offer_prices.sql using jsonb_build_object() to avoid Windows line ending issues.

- **Phase 1 - Text, CSS and Dashboard updates:** Landing page: Added "View full pricing →" link below hero CTA, optimized pricing table for mobile (55%/22.5%/22.5% columns), abbreviated "RAT RATE" to "RAT" on mobile, removed "Most Popular" badge, added gold subtitle "Decline gets a counter. Accept gets an edge." under Accept/Decline + Bonus. Updated Standard vs Rat Rate comparison bullets with new messaging. FAQ: Replaced response time answer with "The Rat works while your league sleeps — most active 10 PM to 5 AM Mountain Time" messaging. Submit page: Added "trade window won't wait" text before submit button. Dashboard: Renamed "Your Credits" → "Active Credits", added "Submit Now" buttons next to each credit tier (pre-selects service on /submit), renamed "Active Bundles" → "Your Purchases", made purchases section collapsible/expandable (collapsed by default), fixed "STANDARD 3 PACK" label showing incorrectly on single-credit purchases. Created CollapsiblePurchases.tsx client component. Fixed server-side error by moving helper functions into client component (can't pass server functions to client components in Next.js 15).

- **Phase 2 - New content, buy buttons, and Trade Finder integration:** Landing page: Added buy buttons to all three "What We Do" service cards (Accept/Decline, Counter Offer, Trade Finder) using BundlePurchaseButton component. Created dedicated Trade Finder section with gold border between services and pricing, including headline "Most trade advice starts with the trade on the table. Ours starts with your entire league," full explanation, 4 checkmarks, buy buttons (Standard $14.99 + Rat Rate $19.99), and link to article page. Pricing page: Removed duplicate price displays above buy buttons for individual services, added turnaround language "Response times may vary — guaranteed within 24 hours." Trade Finder service type: Added 'trade_finder' to BundleConfig interface, BundlePurchaseButton serviceType, CreditSummary component, and submit page state. Users can now purchase, see, and use Trade Finder credits throughout the app. Created app/landing/TradeFinderSection.tsx, updated ServicesSection to client component with hardcoded buy buttons.
