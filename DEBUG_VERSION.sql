-- DEBUG VERSION - This will show exactly where the error occurs
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND OLD.status = 'draft' AND NEW.status = 'submitted') OR
    ...
    ELSE
      ...
      -- THIS IS THE SUSPECTED LINE - let's see if it even gets here
      ...
      WHERE user_id = NEW.user_id
        ...
        AND (
          ...
          (NEW.rate_tier = 'rat_rate' AND bundle_type IN ('standard_3_pack', 'standard_5_pack'))
        )
